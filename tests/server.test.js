...
async function serve() {
  const server = await startGameServer({ port: 0 });
  servers.push(server);
  return server;
}
function get(server, path, method = "GET") {
  return new Promise((accept, reject) => {
    request({ hostname: "127.0.0.1", port: server.address().port, path, method }, (response) => {
      let body = "";
...
describe("local asset server", () => {
  ...
  it("denies repository files and traversal including native Windows forms", async () => {
    const server = await serve();
    for (const path of ["/.git/config", "/package.json", "/src/ui.ts", "/node_modules/vitest/package.json", "/README.md", "/../package.json", "/%2e%2e/package.json", "/dist/../package.json", "/dist/%2e%2e%5cpackage.json", "/dist\\..\\package.json", "/C:/Windows/win.ini", "/dist/ui.js%00", "/dist/ui.js:secret", "//dist/ui.js", "/dist/ui.js.map"]) {
