...
describe("shared economy estimates", () => {
  it("matches resolution for owned discounts and trade bonus", () => {
    ...
    const mutable = {
      ...state,
      ownedTechnologies: [...state.ownedTechnologies],
    };
    const plan = {
    ...
    const { report } = resolveTurn(mutable, plan, 12345, {
      ...CONFIG,
    ...
    expect(report.exportPriceCoins).toBe(
      state.worldPrice * economyRates(mutable, CONFIG).trade,
    );
  });
  it("does not apply newly purchased discounts until the next year", () => {
    ...
    const plan = {
      ...
      preparedHectares: 100,
      storeTons: 0,
      purchaseTechnologies: ["landSurvey" as const, "landSurvey" as const],
    };
    expect(planCosts(state, plan, 0, CONFIG).preparation).toBe(6000);
    expect(planCosts(state, plan, 0, CONFIG).technologies).toBe(
      CONFIG.technologyCosts.landSurvey,
    );
  });
});
