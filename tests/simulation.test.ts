...
describe("resolveTurn", () => {
  ...
  it("grows the population by the growth rate when Consumption is met", () => {
    ...
    expect(result.state.population).toBe(
      Math.round(1_000 * (1 + CONFIG.populationGrowthRate)),
    ); // 1_050
  ...
  it("clamps the plan to the prepared land and floors fractional hectares", () => {
    ...
    expect(fractional.report.harvestTons).toBe(
      123 * CONFIG.baseYieldPerHectare,
    );
  });
...
describe("resolveTurn — fertilizer", () => {
  it("doubles Yield on fertilized hectares and charges the per-hectare cost", () => {
    ...
    expect(result.report.budgetCarryOverCoins).toBe(
      4_000 - seeds - fertilizer - upkeep,
    );
  });
  ...
  it("clamps fertilized hectares to the cultivated area and floors fractions", () => {
    const over = resolveTurn(
      baselineState(),
      {
        cultivatedHectares: 100,
        fertilizedHectares: 9_999,
        preparedHectares: 0,
        storeTons: 0,
      },
      15,
    ...
    // All 100 ha doubled.
    expect(over.report.harvestTons).toBe(
      100 * CONFIG.baseYieldPerHectare * CONFIG.fertilizerYieldMultiplier,
    );
    expect(over.report.fertilizerCostCoins).toBe(
      100 * CONFIG.fertilizerCostPerHectare,
    );
    ...
    expect(fractional.report.harvestTons).toBe(
      38 * CONFIG.baseYieldPerHectare +
        12 * CONFIG.baseYieldPerHectare * CONFIG.fertilizerYieldMultiplier,
    );
...
describe("resolveTurn — land preparation", () => {
  ...
  it("clamps preparation to the remaining Arable land and floors fractions", () => {
    ...
    expect(over.report.landPrepCostCoins).toBe(
      100 * CONFIG.landPrepCostPerHectare,
    );
    ...
    const fractional = resolveTurn(
      baselineState({ budgetCoins: 20_000 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 12.9,
        storeTons: 0,
      },
      23,
      CONFIG,
    );
    expect(fractional.report.landPrepCostCoins).toBe(
      12 * CONFIG.landPrepCostPerHectare,
    );
...
describe("resolveTurn — budget across all spending lines", () => {
  it("charges seeds, fertilizer, and land prep from this Turn's Budget and carries the rest over", () => {
    ...
    expect(result.report.budgetSpentCoins).toBe(
      seeds + fertilizer + prep + upkeep,
    );
    expect(result.report.budgetCarryOverCoins).toBe(
      10_000 - seeds - fertilizer - prep - upkeep,
    );

    // Next-turn Budget = tax on the grown population + unspent carry-over.
    const tax = result.state.population * CONFIG.taxPerPerson;
    expect(result.state.budgetCoins).toBe(
      tax + result.report.budgetCarryOverCoins,
    );
  });
});
...
describe("resolveTurn — config-driven tuning", () => {
  it("obeys the config block: tuning fertilizer and land-prep values changes behaviour", () => {
    ...
    expect(result.report.fertilizerCostCoins).toBe(
      200 * tuned.fertilizerCostPerHectare,
    ); // 1_000
    expect(result.report.landPrepCostCoins).toBe(
      10 * tuned.landPrepCostPerHectare,
    ); // 1_000
  });
});
...
describe("resolveTurn — storage & trade", () => {
  ...
  it("walks the World price by at most the step size, clamped to [min, max], for every seed", () => {
    for (let seed = 0; seed < 200; seed++) {
      ...
      expect(result.state.worldPrice).toBeGreaterThanOrEqual(
        CONFIG.worldPriceMin,
      );
      expect(result.state.worldPrice).toBeLessThanOrEqual(CONFIG.worldPriceMax);
      expect(
        Math.abs(result.state.worldPrice - CONFIG.worldPriceBase),
      ).toBeLessThanOrEqual(CONFIG.worldPriceWalkStep + 1e-9);
    }
  });
  ...
  it("obeys the config block: tuning the walk step and bounds changes behaviour", () => {
    ...
    for (let seed = 0; seed < 20; seed++) {
      expect(
        resolveTurn(baselineState(), faminePlan, seed, frozen).state.worldPrice,
      ).toBe(CONFIG.worldPriceBase);
    }

    const pinned = { ...CONFIG, worldPriceMin: 5, worldPriceMax: 5 };
    for (let seed = 0; seed < 20; seed++) {
      expect(
        resolveTurn(baselineState(), faminePlan, seed, pinned).state.worldPrice,
      ).toBe(5);
    }
  });
  ...
  it("charges storage upkeep on the player's chosen storage, not the auto-stored maximum", () => {
    ...
    expect(storeHalf.report.storageUpkeepCoins).toBe(
      100 * CONFIG.storageUpkeepPerTonPerYear,
    );
    ...
    expect(storeAll.report.storageUpkeepCoins).toBe(
      200 * CONFIG.storageUpkeepPerTonPerYear,
    );
  });
...
describe("resolveTurn — events", () => {
  ...
  it("rolls a deterministic Event for each seed under the seeded RNG", () => {
    ...
    expect(resolveTurn(baselineState(), fullPlan, 0, CONFIG).report.event).toBe(
      "none",
    );
    expect(resolveTurn(baselineState(), fullPlan, 1, CONFIG).report.event).toBe(
      "drought",
    );
    expect(
      resolveTurn(baselineState(), fullPlan, 30, CONFIG).report.event,
    ).toBe("flood");
    expect(resolveTurn(baselineState(), fullPlan, 4, CONFIG).report.event).toBe(
      "priceShock",
    );
    expect(
      resolveTurn(baselineState(), fullPlan, 36, CONFIG).report.event,
    ).toBe("priceShock");
  });
  ...
  it("matches the configured Event probabilities over many seeds", () => {
    ...
    for (let seed = 0; seed < turns; seed++) {
      counts[
        resolveTurn(baselineState(), fullPlan, seed, CONFIG).report.event
      ]++;
    }
  ...
  it("price shock up sells exports at the clamped doubled price", () => {
    ...
    expect(result.state.worldPrice).toBeGreaterThanOrEqual(
      10 - CONFIG.worldPriceWalkStep,
    );
    expect(result.state.worldPrice).toBeLessThanOrEqual(
      10 + CONFIG.worldPriceWalkStep,
    );
  });
  ...
  it("eventSummary names the Event and its concrete effect", () => {
    ...
    expect(eventSummary(drought.report)).toBe(
      "Drought: Yield reduced by 50%, Harvest 600 t",
    );

    const flood = resolveTurn(
      baselineState({ storageTons: 400 }),
      fullPlan,
      30,
      CONFIG,
    );
    expect(eventSummary(flood.report)).toBe(
      "Flood: Yield hit and 100 t of Storage destroyed, Harvest 720 t",
    );
  ...
  it("obeys tuned Event config values", () => {
    ...
    for (const seed of [0, 5, 42]) {
      expect(
        resolveTurn(baselineState(), fullPlan, seed, forcedDrought).report
          .event,
      ).toBe("drought");
    }
...
describe("resolveTurn — technologies", () => {
  ...
  it("Irrigation halves the Drought Yield loss", () => {
    ...
    const withIrrigation = resolveTurn(
      baselineState({ ownedTechnologies: ["irrigation"] }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
  ...
  it("High-yield seeds boost the base Yield on every cultivated hectare", () => {
    ...
    const withSeeds = resolveTurn(
      baselineState({ ownedTechnologies: ["highYieldSeeds"] }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
  ...
  it("Granary halves the Storage upkeep", () => {
    ...
    expect(without.report.storageUpkeepCoins).toBe(
      100 * CONFIG.storageUpkeepPerTonPerYear,
    );

    const withGranary = resolveTurn(
      baselineState({ ownedTechnologies: ["granary"] }),
      fullPlan,
      0,
      CONFIG,
    );
    expect(withGranary.state.storageTons).toBe(100);
    expect(withGranary.report.storageUpkeepCoins).toBe(
      50 * CONFIG.storageUpkeepPerTonPerYear,
    );
  });
  ...
  it("Trade routes raise this Turn's export price without moving the World price", () => {
    ...
    const withRoutes = resolveTurn(
      baselineState({ ownedTechnologies: ["tradeRoutes"] }),
      fullPlan,
      0,
      CONFIG,
    );
  ...
  it("Land survey halves the land preparation cost", () => {
    ...
    const withSurvey = resolveTurn(
      baselineState({ ownedTechnologies: ["landSurvey"] }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 10,
  ...
  it("Fertilizer works halves the fertilizer cost", () => {
    ...
    const withWorks = resolveTurn(
      baselineState({ ownedTechnologies: ["fertilizerWorks"] }),
      fullPlan,
      0,
      CONFIG,
    );
  ...
  it("charges the purchase to this Turn's Budget and applies the effect from the following Turn", () => {
    const plan: PlayerPlan = {
      ...fullPlan,
      purchaseTechnologies: ["highYieldSeeds"],
    };
    const first = resolveTurn(
      baselineState({ budgetCoins: 5000 }),
      plan,
      0,
      CONFIG,
    );
    ...
    expect(first.report.technologyCostCoins).toBe(
      CONFIG.technologyCosts.highYieldSeeds,
    );
    const baseSpend =
      400 * CONFIG.seedCostPerHectare +
      200 * CONFIG.fertilizerCostPerHectare +
      100 * CONFIG.storageUpkeepPerTonPerYear; // 800 + 600 + 100
    expect(first.report.budgetSpentCoins).toBe(
      baseSpend + CONFIG.technologyCosts.highYieldSeeds,
    );
  ...
  it("ignores already-owned Technologies in the purchase list", () => {
    const plan: PlayerPlan = {
      ...fullPlan,
      purchaseTechnologies: ["granary", "irrigation"],
    };
    const result = resolveTurn(
      baselineState({ ownedTechnologies: ["granary"] }),
      plan,
    ...
    expect(result.report.technologyCostCoins).toBe(
      CONFIG.technologyCosts.irrigation,
    );
...
describe("resolveTurn — score", () => {
  ...
  it("raises the Score when a Turn ends above every previous population", () => {
    ...
    const result = resolveTurn(
      baselineState({ population: 1_999, storageTons: 1_200 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
  ...
  it("keeps the previous Score when a Turn ends below it", () => {
    ...
    const grown = resolveTurn(
      baselineState({ population: 2_100, storageTons: 1_500 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
...
describe("resolveTurn — milestones", () => {
  ...
  it("celebrates the Milestone when the population lands exactly on a doubling", () => {
    ...
    const result = resolveTurn(
      baselineState({ population: 1_905, storageTons: 1_200 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
  ...
  it("does not re-fire a Milestone already passed", () => {
    ...
    const result = resolveTurn(
      baselineState({ population: 2_100, storageTons: 1_300 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
  ...
  it("celebrates the quadruple Milestone at four times the starting value", () => {
    ...
    const result = resolveTurn(
      baselineState({ population: 3_900, storageTons: 3_200 }),
      {
        cultivatedHectares: 400,
        fertilizedHectares: 0,
        preparedHectares: 0,
