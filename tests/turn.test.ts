...
const plan = { cultivatedHectares: 400, fertilizedHectares: 400, preparedHectares: 0, storeTons: 0 };
const calm = { ...CONFIG, eventNothingProbability: 1, eventDroughtProbability: 0, eventFloodProbability: 0, eventPriceShockProbability: 0 };
...
it("caps post-harvest storage at the remaining current budget, not export revenue", () => {
  const pending = beginTurn(createNewGame(CONFIG), { ...plan, purchaseTechnologies: ["irrigation"] }, 0, calm);
...
it("retains only surviving old food after a flood", () => {
  const config = { ...calm, eventNothingProbability: 0, eventFloodProbability: 1 };
  const pending = beginTurn({ ...createNewGame(CONFIG), storageTons: 2000 }, { ...plan, cultivatedHectares: 0, fertilizedHectares: 0 }, 0, config);
...
it("reserves mandatory upkeep before accepting production", () => {
  ...
  expect(beginTurn(state, { ...plan, cultivatedHectares: 0, fertilizedHectares: 0 }, 0, calm).minStoreTons).toBe(4000);
});
...
it("reloads a committed outcome and rejects corruption instead of rerolling", () => {
  ...
  expect(parseSave(JSON.stringify({ ...saved, pendingTurn: { ...pendingTurn, maxStoreTons: 999999 } }))).toBeNull();
  expect(parseSave(JSON.stringify({ ...saved, pendingTurn: { ...pendingTurn, result: { ...pendingTurn.result, report: { ...pendingTurn.result.report, exportIncomeCoins: -1 } } } }))).toBeNull();
});

it("freezes storage pricing and checks committed ownership across reload", () => {
  const save = newSave(0);
  const pendingTurn = beginTurn(save.state, plan, 1, calm);
  expect(finishTurn(pendingTurn, 400, { ...calm, storageUpkeepPerTonPerYear: 99 })).toEqual(finishTurn(pendingTurn, 400, calm));
  const corrupt = { ...save, pendingTurn: { ...pendingTurn, result: { ...pendingTurn.result, state: { ...pendingTurn.result.state, ownedTechnologies: ["granary"] } } } };
