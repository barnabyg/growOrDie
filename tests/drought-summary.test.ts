...
it.each([
  ...
  "reports actual drought loss (irrigation %s, multiplier %s)",
  (irrigation, droughtYieldMultiplier, loss, harvest) => {
    ...
    expect(eventSummary(report)).toBe(
      `Drought: Yield reduced by ${loss}%, Harvest ${harvest} t`,
    );
  },
);
