...
it("blocks the 4600-coin commitment against 4000 including upkeep", () => {
  ...
  const plan = { cultivatedHectares: 400, fertilizedHectares: 400, preparedHectares: 0, storeTons: 600, purchaseTechnologies: ["irrigation" as const] };
...
it("permits mandatory retained food and zero spending to recover legacy debt", () => {
  const state = { ...createNewGame(CONFIG), budgetCoins: -600, storageTons: 2000 };
  const result = resolveTurn(state, { cultivatedHectares: 0, fertilizedHectares: 0, preparedHectares: 0, storeTons: 1000 }, 1, calm);
